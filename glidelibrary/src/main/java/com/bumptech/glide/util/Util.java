...
public final class Util {
   ...
   *
   * @see #getBitmapByteSize(ohos.media.image.PixelMap)
   * @deprecated Use {@link #getBitmapByteSize(ohos.media.image.PixelMap)} instead. Scheduled to be
   * removed in Glide 4.0.
   */
  @Deprecated
  public static int getSize(PixelMap bitmap) {
    return getBitmapByteSize(bitmap);
  }

  /**
   * Returns the in memory size of the given {@link PixelMap} in bytes.
   */
  public static int getBitmapByteSize(PixelMap bitmap) {
    ...
    if (bitmap.isReleased()) {
      throw new IllegalStateException("Cannot obtain size for recycled PixelMap: " + bitmap);
    }
    return (int)bitmap.getPixelBytesNumber();
  }

  /**
   * Returns the in memory size of {@link ohos.media.image.PixelMap} with the given width, height, and
   * {@link ohos.media.image.common.PixelFormat}.
   */
