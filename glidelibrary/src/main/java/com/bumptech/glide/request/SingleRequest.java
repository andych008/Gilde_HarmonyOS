...
public final class SingleRequest<R> implements Request,
  ...
  public void begin() {
    ...
    if (model == null) {
      ...
      int logLevel = getFallbackDrawable() <= 0 ? Log.WARN : Log.DEBUG;
  ...
  private int getErrorDrawable() {
    if (requestOptions.getErrorId() > 0) {
      errorDrawable = requestOptions.getErrorId();
    }
    return errorDrawable;
  }

  private int getPlaceholderDrawable() {
    if (requestOptions.getPlaceholderId() > 0) {
      placeholderDrawable = requestOptions.getPlaceholderId();
    }
    return placeholderDrawable;
  }

  private int getFallbackDrawable() {
    if (requestOptions.getFallbackId() > 0) {
      fallbackDrawable = requestOptions.getFallbackId();
    }
    return fallbackDrawable;
  }

  private int loadDrawable(int resourceId) {
    return resourceId;
  }
